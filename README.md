...
## Continaer Runtime Options

By default, the container runtine (`singularity` or `apptainer`) is launched
with the "exec" command, for example:

```bash
[...] singularity exec program [....]
```

If you need to run the container runtime scripts as well you can set the
environment variable RUNTIME_LAUNCH to "run" e.g.

```bash
export RUNTIME_LAUNCH="run"
```

...then the container runtine will be launced with the "run" commnad,
for example:

```bash
[...] singularity run program [....]
```

The programs scripts generated by the `container-mod` script set container
runtine (`singularity` or `apptainer`) option for GPU use.  Generally this is
the only option that needs to be passed to the container runtine.

However, there are occasions when you have to pass extra options to the
container runtime, for example "--sharens" for a shared namespace when using
shared memory for MPI programs.

The environment variable "RUNTIME_OPTIONS" can be used to pass such options
to the container runtime.

A few examples:

```bash
export RUNTIME_OPTIONS="--sharens"
```

```bash
export RUNTIME_OPTIONS="--oom-kill-disable --writable-tmpfs"
```

```bash
export RUNTIME_OPTIONS="--sharens --pem-path="/path/to/RSA/key""
```

Here is a snippet from a Slurm script to run the Quantum Espresso MPI program
"pw.x" with mpirun over multiple nodes

```bash
module load gcc openmpi
module use /path/to/my/modules
module load quantum-espresso/7.3.1-cpu

## Run Quantum ESPRESSO's "pw.x" with shared memory
export RUNTIME_OPTIONS="--sharens"
mpirun --mca pml ob1 --mca btl self,vader,tcp -n ${SLURM_NTASKS} pw.x -in ausurf.in
```
